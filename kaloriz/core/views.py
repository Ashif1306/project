...
def create_snap_token(request):
    ...
    Called via AJAX from checkout review page
    """
    try:
        ...
        if not checkout_data:
            return JsonResponse({
                'success': False,
                'message': 'Data checkout tidak ditemukan. Silakan ulangi proses checkout.'
            }, status=400)
        ...
        # Verify payment method is Midtrans and active
        try:
        ...
        # Check if this is Midtrans payment method
        if 'midtrans' not in payment_method_slug.lower():
            return JsonResponse({
                'success': False,
        ...
        for item in selected_items:
            if not item.product:
                return JsonResponse({
                    'success': False,
                    'message': f'Produk tidak ditemukan.'
                }, status=400)
        ...
        # Calculate amounts
        subtotal = Decimal(cart.get_selected_total() or 0)
        shipping_cost = Decimal(checkout_data.get('shipping_cost', 0))
        ...
        order = Order.objects.create(
            ...
            shipping_address=address,
            selected_courier='',
            selected_service_name=checkout_data.get('shipping_method', 'REG'),
            subtotal=subtotal,
            shipping_cost=shipping_cost,
            total=total,
        )
        ...
        for item in selected_items:
            ...
            OrderItem.objects.create(
                ...
                subtotal=product.get_display_price() * item.quantity,
            )
        ...
        Shipment.objects.create(
            order=order,
            address=address,
            service=checkout_data.get('shipping_method', 'REG'),
            cost=shipping_cost,
            eta=checkout_data.get('eta', ''),
        )

        # Create Snap transaction
        snap_response = midtrans_utils.create_snap_transaction(order, payment_method)

        if not snap_response.get('success'):
            # Delete order if Snap creation fails
            order.delete()
            return JsonResponse({
                'success': False,
                'message': f'Gagal membuat transaksi pembayaran: {snap_response.get("error")}'
            }, status=500)
        ...
        # Clear selected items from cart (we'll restore if payment fails)
        # For now, we keep items until payment is confirmed

        return JsonResponse({
            'success': True,
            'snap_token': snap_response['snap_token'],
            'order_number': order_number,
            'order_url': f'/order/{order_number}/',
        })
...
def midtrans_notification(request):
    ...
    This endpoint will be called by Midtrans server
    """
    try:
        ...
        # Verify signature
        if not midtrans_utils.is_valid_signature(notification_data):
            return JsonResponse({
                'status': 'error',
        ...
        # Parse notification
        parsed_data = midtrans_utils.parse_notification(notification_data)

        # Get order and payment transaction
        order_number = parsed_data['transaction_id']
        try:
        ...
        payment_transaction.transaction_status = parsed_data['transaction_status']
        payment_transaction.fraud_status = parsed_data['fraud_status']
        payment_transaction.payment_type = parsed_data['payment_type']
        payment_transaction.status_code = parsed_data['status_code']
        payment_transaction.signature_key = parsed_data['signature_key']
        payment_transaction.response_midtrans = notification_data
        payment_transaction.save()

        # Update order status based on transaction status
        transaction_status = parsed_data['transaction_status']
        fraud_status = parsed_data['fraud_status']
        ...
        if transaction_status == 'capture':
            if fraud_status == 'accept':
                ...
                # Clear cart items
                _clear_cart_items_for_order(order)
...
def check_payment_status(request, order_number):
    ...
    Used for debugging or manual status check
    """
    try:
        ...
        status_response = midtrans_utils.check_transaction_status(order_number)

        if not status_response.get('success'):
            return JsonResponse({
                'success': False,
                'message': f'Gagal mengecek status: {status_response.get("error")}'
            }, status=500)

        # Update local status
        transaction_data = status_response['data']
        payment_transaction.transaction_status = transaction_data.get('transaction_status')
        payment_transaction.fraud_status = transaction_data.get('fraud_status', '')
        payment_transaction.payment_type = transaction_data.get('payment_type', '')
        payment_transaction.status_code = transaction_data.get('status_code', '')
        payment_transaction.response_midtrans = transaction_data
        payment_transaction.save()

        # Update order status
        if payment_transaction.is_paid() and order.status == 'pending':
            order.status = 'processing'
            order.save()
            _deduct_stock_for_order(order)
            _clear_cart_items_for_order(order)
        ...
        return JsonResponse({
            ...
            'is_paid': payment_transaction.is_paid(),
        })
...
def _clear_cart_items_for_order(order):
    """Helper function to clear cart items after successful payment"""
    cart = Cart.objects.filter(user=order.user).first()
    if cart:
        # Get product IDs from order
        order_product_ids = list(order.items.values_list('product_id', flat=True))
        # Delete corresponding cart items
        cart.items.filter(product_id__in=order_product_ids, is_selected=True).delete()
